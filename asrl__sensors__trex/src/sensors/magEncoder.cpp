...
#include "asrl__sensors__trex/magEncoder.h"
#include <chrono>
#include "sensors/magEncoder.hpp"
...
MagEncoder::MagEncoder()
{
    ...
    lengthPub = nh.advertise< asrl__sensors__trex::magEncoder >("/mag_encoder", 10);
...
void MagEncoder::encoderSettingsToLabJack() {}

void MagEncoder::doCall()
{
    int countMsg = 0;
    ros::Rate rate(200);
    while (ros::ok())
    {
        ...
        srv.request.names[0] = {"AIN2"}; //TODO Use 'DIO2_EF_READ_A_AND_RESET'
        ...
        countMsg+=1;
        asrl__sensors__trex::magEncoder magMsg;
        magMsg.Vout = 1;
        lengthPub.publish(magMsg);
...
int main(int argc, char ** argv) {
    ...
    std::cout << "WE ARE IN MAG!" << std::endl;
	ros::init(argc, argv, "mag_encoder_cpp_test");

    std::shared_ptr<MagEncoder> motorEncoder(new MagEncoder());
