...
class LabjackInterfaceNode:
	...
	def dataReadSPI(self,readRequest):
		try:
			...
			#print "readRequest.TX_MSG : ", readRequest.TX_MSG
			#print "readRequest.NUM_BYTES : ", readRequest.NUM_BYTES
			#print "SPI_GO : ", readRequest.SPI_GO
			...
			if globalConfigData.use_new_dode_spi_multi_read :
				...
				# Update : It looks like setting eWriteNames(self.handle, 1, ...) is not correct.
				#	From the api : https://labjack.com/support/software/api/ljm/function-reference/ljmewriteaddresses,
				# 	we can see that NumFrames=(size of aAddresses). 1 is obviously not the correct size.
				# 	a. If we set it to the correct size, 8, then we do not get empty message response on a power-cycle.
				#	Print out the "response" to see that its not empty.
				# 	b. If we change it eWriteNames(self.handle, 1, ...), and do a power-cycle, we can see that
				#	the response is empty (vector of all 0s). Why? the NumFrames is not correct.
				# 	Strange behaviour to investigate : If we first set it to "8" and then we set it to "1", the
				# 	messages are returning correct response numbers. How is that happening when "1" is not correct.
				# 	Is lj api caching some information (say it knows the size from the initial read)?
				ljm.eWriteNames(self.handle, 8,
					["SPI_CS_DIONUM",
					 "SPI_CLK_DIONUM",
					 "SPI_MISO_DIONUM",
					 "SPI_MOSI_DIONUM",
